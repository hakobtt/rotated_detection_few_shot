import torch
from mmdet_custom.core.bbox.transforms_rbbox import gt_mask_bp_obbs

from ..bbox import (PseudoSampler, assign_and_sample, bbox2delta,
                    build_assigner, build_sampler, dbbox2delta, dbbox2delta_v3,
                    hbb2obb_v2)
...
def anchor_target_rbbox_single(flat_anchors,
    ...
    if sampling:
        if hbb_trans == 'obb2obb':
            bbox_assigner = build_assigner(cfg.assigner)
            bbox_sampler = build_sampler(cfg.sampler)
            assign_result = bbox_assigner.assign(anchors, gt_obbs, gt_bboxes_ignore,
                                                gt_labels)
            sampling_result = bbox_sampler.sample(assign_result, anchors, gt_obbs_ts,
                                                gt_labels)
        else:
    ...
    if len(pos_inds) > 0:
        ...
        if hbb_trans == 'obb2obb':
            pos_ext_bboxes = sampling_result.pos_bboxes
